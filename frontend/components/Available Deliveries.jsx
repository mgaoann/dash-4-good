...
export default function AvailableDeliveries({ items = [], onClaim }) {
        ...
        <View style={styles.extraInfo}>
          <View style={styles.infoGroup}>
            <View style={styles.infoItem}>
              ...
              <Text style={styles.infoText}>{item.estimatedTime || "N/A"}</Text>
            </View>
          </View>
          <Text style={styles.organization}>{item.organization}</Text>
        </View>

        {/* Action triggers the passed-down function */}
        <Button
          title="Claim Delivery"
          onPress={() => onClaim && onClaim(item)}
          style={{ marginTop: 10 }}
        />
      </View>
    );
...
const styles = StyleSheet.create({
  section: { marginTop: 9, paddingHorizontal: 0 },
  sectionTitle: {
    fontSize: 16,
    fontWeight: "bold",
    color: "#1F2937",
  ...
  title: { fontWeight: "600", fontSize: 16, color: "#1F2937" },
  badge: {
  ...
  infoGroup: { flexDirection: "row", alignItems: "center", gap: 16 },
  infoItem: { flexDirection: "row", alignItems: "center" },
  infoText: { fontSize: 12, color: "#4B5563", marginLeft: 4 },
